...
echo "Добавляем и считываем в redis_master тестовое значение"
docker compose exec -T redis_master redis-cli <<EOF
...
echo "Считываем в redis_slave тестовое значение"
docker compose exec -T redis_slave redis-cli <<EOF
get testkey
EOF

echo "Проверяем скорость запросов с redis"
curl  -kv -w '\n* Response time: %{time_total}s\n' \
  -X 'GET' \
  'http://localhost:8080/helloDoc/users/username2' \
